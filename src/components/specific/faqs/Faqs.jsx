...
export const Faqs = ({ estilos }) => {
	...
	return (
		<div className="FaqsData-content">
			<div className="FaqsData-wrapper">
				{estilos.map((producto, index) => {
					const isActive = accordion[index];
					const className = `FaqsData-info ${isActive ? "active" : ""} faq-${
						producto.id
					}`;
					...
					return (
						<div className={className} key={producto.id}>
							<div
								className="FaqsData-title-main Font-head Fbold"
								onClick={() => accordionToggle(index)}
							>
								<div className="FaqsData-num">{index + 1}</div>
								<div className="FaqsData-title">
									{producto.titulo}
									{isActive ? (
										<IoIosArrowDown className="FaqsData-iconBottom" />
									) : (
										<IoIosArrowUp className="FaqsData-iconUp" />
									)}
								</div>
							</div>
